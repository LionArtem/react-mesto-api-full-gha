...
export default function InfoTooltip({ infoToolTipText, onClose }) {
  return (
    <div
      onClick={(evt) => closeOverley(evt, onClose)}
      className={infoToolTipText ? 'popup  popup_opened' : 'popup'}
    >
      <div className="popup__container">
        ...
        'Что-то пошло не так! Попробуйте ещё раз.' ? (
          <svg
            className="popup__registration-picture"
            width="120"
            height="120"
            viewBox="0 0 120 120"
            fill="none"
        ...
        <p className="popup__registration-text">{infoToolTipText}</p>
      </div>
    </div>
  );
}
